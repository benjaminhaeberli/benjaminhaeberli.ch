...
<?php if (KirbyHelper::siteHasField('megamenu')) : ?>
    <nav class="container flex max-w-screen-xl gap-2 pb-4 mx-auto text-lg font-semibold">
        <ol class="flex gap-2">
            <?php
            ...
            foreach ($links as $link) : ?>
                ...
                <?php if (($page = KirbyHelper::isPage($page)) !== null) : ?>
                    <li class="flex">
                        <?php if (page()->is($page)) : ?>
                            <a href="#" class="flex items-center gap-2 px-2 py-2 group focus:bg-transparent text-zinc-950 bg-primary hover:bg-slate-800 focus:text-slate-200 focus:outline-dashed outline-2 ">
                            <?php else : ?>
